...
class Trainer(BaseTrainer):
    ...
    def train_step(self, data, it=None):
        ...
        '''
        loss_gen, deterministic_loss, gloss_fake, gloss_mix, ploss_mix, cam_pred_loss, recon_loss = self.train_step_generator(data, it)
        loss_d, reg_d, real_d, mix_d, fake_d, real_p, mix_p = self.train_step_discriminator(data, it)
    ...
    def eval_step(self, data):
        ...
        for i in tqdm(range(n_iter)):
            with torch.no_grad():
                x_fake, x_mix, pred_cam = generator(x_real, pose_real)
        x_fake.clamp_(0., 1.)
    ...
    def train_step_generator(self, data, it=None, z=None):
        ...
        if self.multi_gpu:
            ...
            x_fake, x_mix, pred_cam = generator(x_real, pose_real, **latents)        
        ...
        deterministic_loss = cam_pred_loss + recon_loss
        ...
        gen_loss = deterministic_loss + ((gloss_fake + gloss_mix)*0.5 + ploss_mix * self.patch_discriminator.lambda_PatchGAN)
        ...
        return gen_loss.item(), deterministic_loss.item(), gloss_fake.item(), gloss_mix.item(), ploss_mix.item(), cam_pred_loss.item(), recon_loss.item()
    ...
    def train_step_discriminator(self, data, it=None, z=None):
        ...
        loss_d_full += d_loss_mix * 0.5

        x_fake.requires_grad_()
        d_fake = discriminator(x_fake)
        d_loss_fake= compute_bce(d_fake, 0)
        loss_d_full += d_loss_fake * 0.5
        ...
        d_loss = (d_loss_real + (d_loss_fake + d_loss_mix)*0.5) 

        return (
            d_loss.item(), reg.item(), d_loss_real.item(), d_loss_mix.item(), d_loss_fake.item(), ploss_real, ploss_mix)
    ...
    def visualize(self, data, it=0, mode=None, val_idx=None):
        ...
        return image_fake, shape_rgb, appearance_rgb, swap_rgb
        """
