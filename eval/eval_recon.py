...
out_txt_file = os.path.join(output_dir,'metrics.txt') 
...
print("finished rendering", len(recon_image), "images")
...
for i in range(len(img_fake)):
    gt = batch[i].numpy().transpose(1, 2, 0)
    pred = img_fake[i].numpy().transpose(1, 2, 0)
...
with torch.no_grad():
    for predi, gti in zip(preds, gts):
...
psnr_avg /= num_ims
ssim_avg /= num_ims
out_txt = "fid  {}\npsnr {}\nssim {}\nlpips {}".format(fid_score, psnr_avg, ssim_avg, lpips)
print("writing")
